...
{
    //reset servos back to slightly toed-in
    int RxC=1475;
    int RyC=1380;
    int LxC=1500;
    int LyC=1600;
	...
	//Reduce the max number of disparities to make the disparity map fill more of the window. Increase the blocksize to smooth out the image
    cv::CommandLineParser parser(argc, argv,
                                 "{@arg1||}{@arg2||}{help h||}{algorithm||}{max-disparity|96|}{blocksize|3|}{no-display||}{scale|1|}{i||}{e||}{o||}{p||}");
    if(parser.has("help"))
    {
        print_help();
        return 0;
...
*/
    ...
    if( !intrinsic_filename.empty() )
    {
        ...
        while (inLOOP){
            ...
            //disp=FrameFlpd( Rect(0, 0, 640, 480));
