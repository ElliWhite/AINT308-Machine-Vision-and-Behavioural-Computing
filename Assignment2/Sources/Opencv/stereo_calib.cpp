...
// OR ELSE HARTLEY'S METHOD
 ...
 // from the fundamental matrix
    ...
    for( i = 0; i < nimages; i++ )
    {
        ...
        char c = (char)waitKey();	//wait until any key pressed
        if( c == 27 || c == 'q' || c == 'Q' )
            break;	//quit
...
void frameCapture(VideoCapture cap2, string source2, Mat *LeftCopy, Mat *RightCopy)
{
    if (!cap2.read(Frame))
    {
...
int main(int argc, char** argv)
{
    ...
    CMDstream.str("");
    CMDstream.clear();
    CMDstream << RxC << " " << RyC << " " << LxC << " " << LyC << " " << NeckC;
    CMD = CMDstream.str();
	...
	//stringstream to hold name of the file
	...
	//loop to take 20 images and save them
    while(inLOOP){

		//capture frame
        frameCapture(cap,source,&Left_Image,&Right_Image);
		
		//check to see if the spacebar was pressed. If so, save images
        if(key == 32){
            
			//create the file name to be saved
            name << "C:/Repository/Data/CalImg/Left_Image_" << i << ".png";	
			
			//save the left image 
            cv::imwrite(name.str(), Left_Image);

			//reset the stringstream
            name.str("");

            //create the file name to be saved
            name << "C:/Repository/Data/CalImg/Right_Image_" << i << ".png";
			
			//save the right image
            cv::imwrite(name.str(), Right_Image);
		...
		//if 20 images have been saved then break from loop
        if (i == 20){
            inLOOP = false;
        }
